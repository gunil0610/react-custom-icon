...
import { readSvgDirectory } from "@repo/helpers";
...
const ICONS_DIR = path.resolve(process.cwd(), "../../svgs"); // Path to the icons directory
const OUTPUT_DIR = path.resolve(
  process.cwd(),
  cliArguments.output || "../build"
);
...
async function buildIcons() {
  ...
  const { default: iconFileTemplate } = await import(
    path.resolve(process.cwd(), templateSrc)
  );
