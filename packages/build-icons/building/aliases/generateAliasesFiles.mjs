...
import { toPascalCase, resetFile, appendFile } from "@repo/helpers";
...
export default async function generateAliasesFiles({
  ...
  await Promise.all(
    icons.map(async (iconName, index) => {
      ...
      if (
        (iconAliases != null && Array.isArray(iconAliases)) ||
        !aliasNamesOnly
      ) {
  ...
  if (showLog) {
    ...
    console.log(
      `Successfully generated src/aliases/${aliasPrefixesFileName} file`
    );
    console.log(
      `Successfully generated src/aliases/${aliasSuffixFileName} file`
    );
  }
}
